...
public class ManagerWebService {
	...
	public Manager create(@Context HttpServletRequest request, Manager order) {
		HttpSession session = request.getSession();
		session.setAttribute("Manager", "TESTE");
		return null;
	}

	@GET
	@Produces(MediaType.APPLICATION_JSON)
	public Manager list(@Context HttpServletRequest request) {
		return null;
	...
	@Path("{id}")
	@Produces(MediaType.APPLICATION_JSON)
	public Manager update(@Context HttpServletRequest request, @PathParam("id") int id) {
		HttpSession session = request.getSession();
		session.setAttribute("Manager", "TESTE");
		return null;
	}
	...
	static class ManagerWebService_JerseyClient {

		private WebTarget webTarget;
		private Client client;
		private static final String BASE_URI = "http://localhost:8080/TrueTravel/";

		public ManagerWebService_JerseyClient() {
